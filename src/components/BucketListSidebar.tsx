...
export const BucketListSidebar: React.FC<BucketListSidebarProps> = ({
    ...
    const shareMap = async () => {
        ...
        try {
            ...
            const platformChoice = await new Promise<string | null>(
                (resolve) => {
        ...
        `;
            ...
            try {
                ...
                toast({
                    title: "Text Copied!",
                    description:
                        "Share text copied to clipboard. Paste it anywhere!",
                });
    ...
    const getBadges = () => {
        const badges = [];
        if (completedCount >= 1)
            badges.push({ name: "Explorer", icon: "🎯", color: "secondary" });
        if (completedCount >= 5)
            badges.push({ name: "Adventurer", icon: "🏃‍♂️", color: "primary" });
        ...
        <div className="w-full h-full bg-sidebar border-r border-sidebar-border flex flex-col">
            ...
            <div className="p-3 md:p-4 border-b border-sidebar-border bg-gradient-ocean relative">
                <Button
                    ...
                    title="Close sidebar"
                    className="absolute top-2 right-2 rounded-md bg-black/20 text-white backdrop-blur-md shadow-md ring-1 ring-white/20 hover:bg-white/20 hover:shadow-lg transition-transform hover:scale-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/60"
                >
                    <ChevronLeft className="h-4 w-4" />
                </Button>
                <h1 className="text-2xl md:text-xl font-bold text-primary-foreground pr-10">
                    Bucket List
                </h1>
                <p className="text-xs md:text-sm text-primary-foreground/80 mt-2">
                    Click anywhere on the map to add a new destination!
                </p>
            </div>
                ...
                <div className="p-3 md:p-4 space-y-2 md:space-y-3">
                    {bucketList.length === 0 ? (
                        <div className="text-center py-8 text-sidebar-foreground/60">
                            ...
                            <p className="text-xs">
                                Click on the map to start your adventure.
                            </p>
                        </div>
                    ) : (
                        bucketList.map((item) => (
                            <Card
                                ...
                                } transition-opacity bg-sidebar-accent border-sidebar-border`}
                            >
